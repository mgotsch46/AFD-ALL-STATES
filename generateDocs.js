...
function r(text, opts = {}) {
  return new TextRun({ text: String(text || ''), font: ARIAL, size: opts.size || 20, bold: opts.bold || false,
...
function buildAgreementForDeed(d) {
  ...
  buyers.forEach((b, i) => {
    ...
    children.push(p([r(b + (email ? '   Email: ' + email : '') + (phone ? '   Phone: ' + phone : ''), { bold: false })]));
...
function buildTIL(d) {
  ...
  const loanTermDisplay = d.loan_term_months ? (d.loan_term_months + '/' + Math.round(d.loan_term_months / 12)) : '360/30';
  const children = [
    ...
    new Table({
      ...
      rows: [
        ...
        new TableRow({ children: [cell('Loan Term (months/years):', true, 3200), cell(loanTermDisplay, false, 6880)] }),
    ...
    p([r('Number of Payments: ' + (d.loan_term_months || '360') + '   \u2014   Amount of Each Payment: '), r(d.pi_numbers, { bold: true })]),
    blank(),
...
function buildLeadPaint(d) {
  ...
  const children = [
    ...
    blank(),
    p([r('Certification of Accuracy'), r(' The following parties have reviewed the information above and certify, to the best of their knowledge, that the information they have provided is true and accurate.')]),
    blank(),
    hRule(),
...
function buildMemorandum(d) {
  ...
  const children = [
    ...
    p([r(d.seller_name)]),
    p([r('By:')]),
    p([r((d.signor || d.seller_name) + ' \u2014 Member/Manager')]),
    blank(),
  ...
  const children = [
    ...
    new Table({
      ...
      rows: [
        ...
        ...items.map((item, i) => new TableRow({ children: [chkCell('', 900), chkCell('', 900), chkCell('', 900), chkCell((i+1) + '.  ' + item, 7380)] }))
      ]
    }),
    blank(),
